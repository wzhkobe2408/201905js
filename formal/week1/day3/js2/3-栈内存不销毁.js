...
* */
// 栈内存不销毁的特殊情况：
// 一般情况下，函数执行完，栈内存就会销毁。但是函数执行时，函数作用域中有一些值被其他地方占用，此时作用域不能销毁（栈内存不销毁）；
...
var obj = add(); // add() 执行后返回的对象被obj所接收，所以add执行的作用域不能被销毁，所以栈内存不能释放，保存在作用域中的变量n的值也不会被销毁。
obj.addNum(); // 2
obj.addNum(); // 3
add(); // 这样执行add函数，没有人接收它的返回值，执行完后，add的作用域也就销毁了。
function foo() {
  var n = 1;
  n++;
  console.log(n);
...
// obj2的getF4这个属性占用着f4的返回值，所以f4执行时，作用域不销毁。
