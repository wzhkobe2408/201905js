...
// this 是一个js的关键字，代表当前代码【执行】的环境对象。一般在函数中使用，在函数执行时确定，根据函数执行方式的不同，代表的值不同。目前有以下情况：
...
// obj.fn(); // fn前面有 . 点前面是obj所以本次obj.fn执行时，obj.fn中的this就是obj
// obj['fn'](); // 注意 obj['fn'] 等效于 obj.fn，所以本次执行 obj.fn中的this仍然是obj

var fn = obj.fn;
fn(); // 函数执行时（最后有小括号()）函数名前面没有点，所以函数里面的this就是window
...
// 箭头函数的this指向是箭头函数声明时所在作用域中的this
