...
// 1. 构造函数只能用new来调用（用new调用，这个函数才叫构造函数）
// 2. 构造函数不需要手动创建实例对象，在通过new调用构造函数时，构造函数会隐式的创建一个属于这个类的实例对象，并且把构造函数中的this指向这个实例对象。后续的加工都在this上加工就可以了。
...
// 构造函数模式：像上面这种，把创建对象的细节封装到构造函数中。通过this.xxx = xxx向实例对象上添加属性。用这种方式创建对象称为构造函数模式。此时构造函数就叫做类，函数名叫类名(一般情况下类名首字母大写，如果有多个单词，使用驼峰命名法)；
...
let ary2 = new Array(1, 2, 4);
let ary3 = new Array(1, 3, 5);
...
// 构造函数虽然解决了工厂函数没有类型的问题，但是相比较Array内置类，我们自定义的类Teacher没办法让共有的属性和方法共有，而是每个实例都自己有一份。例如t2和t3都是老师类的实例，而只要是老师就应该会teach（教学），说明teach是老师类共有的属性，但是构造函数Teacher却没办法实现？
