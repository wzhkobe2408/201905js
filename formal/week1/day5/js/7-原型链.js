...
// 当我们访问一个实例对象的属性时，浏览器会首先在实例对象的私有属性中查找，如果找到就使用这个私有属性，如果没找到，通过实例对象__proto__属性去当前实例对象所属类的prototype（原型）上查找，如果在原型的公有属性中找到这个属性，就使用这个公有的属性。如果找不到，就通过prototype（原型）对象继续向上查找，一直找到基类Object的原型上，如果还没找到，就返回undefined
