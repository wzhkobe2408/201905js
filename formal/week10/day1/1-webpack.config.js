let HtmlWebpackPlugin = require('html-webpack-plugin'); // html-webpack-plugin 是第三方的依赖，需要单独安装
...
module.exports = {
  ...
  devServer: { // webpack-dev-server 是webpack在开发中开启的本地服务
    ...
    proxy: {
      '/api': { // /api 就是一个标识符，如果接口中包含 /api 就采用对应的代理策略
        ...
        changeOrigin: true, // 当target是一个域名时要设置
  ...
  plugins: [
    new HtmlWebpackPlugin({
      template: __dirname + '/public/index.html' // 会按照这个模板生成html，并且把打包后的文件引入进去
    })
  ]
};