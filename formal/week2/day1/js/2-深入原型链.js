...
// 原型（prototype）：函数都有一个prototype的属性，并且它的值是一个对象；如果这个函数被用来当做类（构造函数）来使用，此时prototype是用来存放这个类的公有属性和方法的；
// 原型链：每个对象（实例对象、prototype对象）都有一个__proto__属性。当我们 对象.属性名 时，会首先在私有属性中查找，如果私有属性没有，就通过实例的__proto__ 去当前实例所属类的原型上查找，如果原型上还没有，就通过原型__proto__继续向上查找，一直找到Object的原型，如果还没有就返回undefined；；
...
console.log(f1);
f1.say();*/
// 实例__proto__ 不仅是只读的，还是可写的，所以不要轻易的修改这个属性；
...
console.log(Fn.prototype.constructor); // Object 因为我们在修改Fn.prototype指向obj时，obj没有constructor，所以通过obj的__proto__找到Object的prototype拿到Object构造函数；
