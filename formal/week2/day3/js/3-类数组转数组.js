...
// 2. 把类数组对象通过扩展运算符展开到一个数组中；ES6新增的方法，老旧浏览器不兼容
...
// 3. 借用数组原型上的slice方法
// 借用Array.prototype.slice() 方法，在slice执行时，把slice中的this修改类数组，就可以实现把类数组转成数组；
...
// 简写方式：IE低版本会报错；
let ary3 = [].slice.call(divList); // Array.prototype.slice 直接访问Array的原型对象的slice方法；[].slice 使用[] 空数组的原型链查找机制找到slice方法；
// console.log(ary3);
