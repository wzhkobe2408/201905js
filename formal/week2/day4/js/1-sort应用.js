...
// 1. sort不传参数时也可以排序，只有排0-9之间的数字时，才能正常排序；

var ary = [1, 2, 14, 5, 8, 0, 15, 13, 16];
var ary2 = [0, 3, 1, 9, 5, 7];
var r1 = ary.sort();
// console.log(ary);
// console.log(r1);
...
// a, b 都是数组中的项，回调函数被执行多次。回调函数执行次数和数组项数、以及数组项本身排列顺序有关；
...
// 3. 给listAry排序：listAry 中数组项都是元素对象，而我们需要按照页面中标签里面的数字排序，而数字恰好就是元素对象的innerHTML属性。所以这个问题就转变成了按照每个li的innerHTML属性进行排序
listAry.sort((a, b) => {
...
for (let i = 0; i < listAry.length; i++) {
  box.appendChild(listAry[i]); // 把每一项重新添加要页面中？
}
...
// 3. 给hwAry按照价格降序排序：hwAry中都是li元素对象，怎么获取到存储在行内属性中价格？
