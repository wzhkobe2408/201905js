...
// JSON 数据：不是数据类型，是一种常用的前后端交互的数据格式。

// -> 1. JSON格式的对象：把属性名和属性值都用字符串包起来，如果属性值数字可以不包（如果用引号包了，就是字符串）；
var obj = {
  "name": "张三"
};
...
// js中引号在字符串中单双循环着用；如果最外层是单引号，字符串中不能再用单引号了，如果一定要用要使用 \' (\叫做转义符); 如果最外层是双引号，字符串中不能再用双引号，如果一定要用必须使用 \" 转义

let str1 = "{\"name\": \"张三\"}";
let str2 = '[{"name": "张三"}, {"name": "李四"}]';

// JSON格式的对象和JSON格式的字符串互转
// 在window上面有一个JSON的对象，其中有两个方法；
...
let ary5 = JSON.parse(JSON.stringify(ary3)); // 先把对象转成JSON格式的字符串，再把JSON格式的字符串转成一个对象，这个对象和原来的没有关系；

ary5[1].name = '李四';
// console.log(ary3);
// console.log(ary5);

// IE 6/7 没有JSON对象，无法使用JSON.parse()把字符串转成对象
// 在IE 6/7中使用eval方法
