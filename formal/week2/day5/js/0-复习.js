/*
* 1. sort 应用：
*   1.1 sort不传参数也可以排序，但是只能正常排0-9的数字
*   1.2 sort(callback)
*     1.2.1 普通数字升序 sort(function (a, b) {return a - b})
*     1.2.2 排二维数组 sort(function (a, b) {return a.age - b.age}) 按照数组项的age属性升序排序；排序的时候整体移动数组项，不是只排age属性
...
*
* 2. 数组复制
*   数组项是基本数据类型，数组存储的这一项就是这个值本身，在复制时，复制的也是这个值本身；所以复制出来的这一份和原来的数组没关系；
*   数组项是引用数组类型，数组存储的是一项的堆内存地址，在复制时，复制的是这个对象的堆内存地址，后续如果操作新数组中的项，其实在操作这个地址，所以原来的数组也会被修改。（浅复制、浅拷贝）；
*
* 3. DOM映射：页面中的html元素和通过js的相关方法（getElementBy一切）获取来的对象或者集合存在映射关系（一个改变另一个跟着改）；
*   1. box2.style.backgroundColor = 'red'
*   2. appendChild(元素对象) 它会先看一下这个要追加到容器中的元素是否存在于页面中，如果这个元素已经存在了，它直接把元素移动到容器末尾。
*   3.
*
...
*     http method "GET"
*     请求接口地址 一般服务端（后端）的人提供一个链接 称为 接口
*     异步还是同步：false同步，true表示异步，默认异步
*   xhr.open("GET", "json/data.json", false);
*   5.3 监听xhr的onreadystatechange（on ready state change） 事件
