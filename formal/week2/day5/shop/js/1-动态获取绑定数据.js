...
// 2. 获取和绑定数据：真实项目中的数据大多数情况下不是写死的。而是通过ajax、jsonp等技术从服务端获取数据，然后把获取到的数据动态的绑定到页面中；
// 2.1 基于ajax从服务端获取数据（我们本次没有服务端，所以请求本地的json文件，真实项目中请求的都是服务端提供的接口）；
// 2.2 把上一步拿到的数据搞成html结构（动态创建DOM或者拼接字符串）;
// 2.3 把上一步得到的html结构插入页面中去
...
// url: 请求的接口地址
// async: false 表示同步，true表示异步
xhr.onreadystatechange = function () {
  if (xhr.readyState === 4 && xhr.status === 200) {
    // xhr.readyState 是表示当前请求状态的值
    // xhr.status 这个status是http status , 以2开头的数字都是成功
    // console.log(xhr.responseText); // xhr.responseText 存储的是服务端返回的数据
...
// DOM回流、重绘
// DOM回流（reflow）：页面中的元素增加、删除、大小、位置的改变，会引起浏览器重新计算其他元素的位置，这种现象称为DOM回流。DOM回流非常消耗性能，尽量避免DOM回流；
...
// 在数据绑定之前，ul下面并没有li，所以我们获取来的集合productList是一个空集合。但是当数据绑定完成后，我们并没有重新获取这个ul下面的li，但是因为DOM映射的关系，页面中ul下面有li了，DOM映射会把新插入的li映射到productList集合中，所以这个集合不是空集合了。但是querySelectorAll()方法获取来的集合是静态集合 staticNodeList，是掐断了DOM映射的；如果你在绑定前用querySelectorAll方法获取的元素集合，需要在数据绑定后重新获取。