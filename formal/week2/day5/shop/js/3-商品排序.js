...
// 2. 封装一个处理排序的方法
// let flag = false; //
// let flag = -1; // 我们发现一个问题，当我们第一次点击的是上架时间时，商品按照上架时间进行了升序排序，但是接下来点击价格，我们期待商品按照价格升序排序，事实上是商品按照价格的降序排列了。之所以会有这种情况，因为第一次点击上架时间时，全局变量flag被修改成+1了，当点击价格的时候又给全局变量乘了一个-1，所以此时flag变成-1，再乘给 (aInn - bInn) 时就变成 bInn - aInn，即降序；为了解决这个问题，我们让每个a标签都拥有自己的flag，在点击的时候把自己的乘等于-1；

let sortList = function (index, flag) {   // 用let声明的变量必须在声明后才能使用

  // index 形参代表的是被点击a标签的索引，用来判断按照哪个维度排序；如果索引为0表示按照上架时间排序，1表示价格，2表示热度
  // flag 表示排序标识符，函数被调用时传入，1 表示升序，-1表示降序
...
for (let i = 0; i < linkList.length; i++) {
  ...
  linkList[i].onclick = function () {
    // 还有一个功能，点击当前维度时当前维度按照升序排序，当我再点击其他维度时，不管上一次是升序还是降序，本次就要升序。所以需要点击当前维度时，重置其他维度的flag为-1；
    ...
    this.flag *= -1;
    sortList(i, this.flag); // 点击每个按钮都会调用sortList方法排序，sortList方法需要知道被点击的是哪个按钮，按照哪个维度排序；
    // 此时我们把flag属性变成了每个a标签私有属性，sortList方法依赖flag，所以我们直接在执行sortList方法时，把a标签的flag属性作为实参传递给sortList方法
  };
}
