...
* \s 匹配一个空白符（空格、制表符\t）
* \b 匹配边界，'zhu-feng' 边界字母和非字母连接的地方，z的左边，u的右边，f的左边，g的右边都是边界
* \n 匹配一个换行符
* . 除换行符\n以外的任意一个字符
...
// 普通元字符：除特殊元字符、量词元字符以外的其他元字符都是普通元字符；
...
// RegExp.prototype.test() 方法：这个方法用来匹配，检测一个字符串是否符合当前正则，如果符合，就会返回true，不符合返回false
