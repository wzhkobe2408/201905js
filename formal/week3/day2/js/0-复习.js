...
* \w 数字、字母、_中的任意一个
* \s 匹配一个空白符(空格、制表符\t)
* \b 匹配边界（boundary 边界）'zhu-feng' 字母和非字母连接处成为边界
* \n 匹配一个换行符
* .  匹配除换行符\n 以外的任意一个字符
* \ 转义符，把有特殊意义的元字符转义成原本的意义。例如 . 表示除换行符以外的任意一个字符，\. 表示普通小数点。还可以把普通元字符转成特殊元字符，例如\d 表示0-9的数字
*
* ^(读作caret) 表示以某个元字符开头
* $ 表示以某个元字符结尾
* x|y 表示x或者y中的一个
...
// test方法用来做正则匹配，如果匹配返回true，不匹配返回false；
let reg = /\d/; // 0-9的任意一个数字
...
// console.log(reg5.test('我的')); // false
...
// console.log(reg9.test('16601046931'));

// let reg10 = /ing$/; // 必须以g结尾，但是g的前面还要有in
let reg10 = /g$/; // 必须以g结尾
...
let reg13 = /[a-z]/;
// console.log(reg13.test('123')); // false
// console.log(reg13.test('x')); // true

let reg14 = /[^abc]/; // 除了a/b/c啥都行
// console.log(reg14.test('a')); // false
// console.log(reg14.test('b')); // false
// console.log(reg14.test('c')); // false
// console.log(reg14.test('999')); // true

let reg15 = /a*/; // * 表示0到多次
// console.log(reg15.test('abc')); // true
// console.log(reg15.test('bcd')); // true
// console.log(reg15.test('aaabcd')); // true

let reg16 = /b+/; // + 至少出现一次
// console.log(reg16.test('acd')); // false
// console.log(reg16.test('bbb')); // true

let reg17 = /b?/; // 0次到1次
// console.log(reg17.test('acd')); // true
// console.log(reg17.test('bbbb')); // true

let reg18 = /^\d{3}$/;
// console.log(reg18.test('256')); // false
// console.log(reg18.test('111')); // true

let reg19 = /^\d{3,}$/; // 至少出现3次
// console.log(reg19.test('123')); // true
// console.log(reg19.test('1234')); // true
// console.log(reg19.test('12')); // false

let reg20 = /^\d{3,5}$/; // 3次到5次
console.log(reg20.test('123')); // true
