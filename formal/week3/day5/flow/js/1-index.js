...
// 为什么页面中的图片插入的时候会偶尔乱序？因为for循环在向页面中的列插入时是同步插入的，但是图片加载时异步的，就是说向列中追加数据时，不会等着图片请求回来再插。这样做就会导致图片还没拿到时，图片高度就是0，这张图片所在的列就被当成最矮的列，在追加的时候又优先给这一列追加了一张。等那张图片加载完，高度就会被撑开，所以这一列很有可能比别人多了图片。
