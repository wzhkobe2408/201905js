...
// 1. 设置CSS样式：元素.style.xxx = xxx 的方式设置元素的样式；
function setCss(ele, attr, val) {
  // 处理单位：width/height/margin/padding/left/top/bottom/right/fontSize...
  // 处理属性是上面的这些属性，值是不带单位的数字
  let reg = /(fontSize|width|height|(margin|padding)?(top|right|bottom|left)?)/i; // 写i的原因是为了匹配marginTop这些属性
...
setCss(box, 'marginTop', 100); // 想好你的方法要怎么调用，组织参数
...
function setBatchCss(ele, batch) {
  if (typeof batch !== 'object') {
    throw TypeError('\'batch\' is not a object');
  }
...
// 判断获取的是元素集合还是元素：扩展内容不要求掌握
// 判断获取的结果是 HTMLCollection 或者NodeList的实例就是集合
...
function hasClass(ele, className) {
  ...
  return ele.className.indexOf(cN) !== -1; // indexOf查找字符首次在字符串中出现的索引位置，如果出现过就是索引，如果没出现过返回-1；
}
hasClass(box, ' box');
