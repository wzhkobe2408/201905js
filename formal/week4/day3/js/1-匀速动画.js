// 需求：让页面中的div#box在2s内匀速运动到left为800的位置；并且在动画到达终点时console.log('终于跑完了')

let box = document.querySelector('#box');

const { css } = window.utils; // 从utils中把css方法解构出来
// css(box, 'width') 获取box的width样式的值
// css(box, 'width', 100) 设置box的width样式的值为100
// css(box, {width: 100, height: 100}) 批量设置box的样式

// 过渡时间
let duration = 2000; // 动画运动时间
...
let timerId = setInterval(() => {
  ...
  if (curT >= duration) { // 满足这个条件时表示元素已经到达或者超过终点
    ...
    console.log('终于跑完了'); // 如果有一些事情需要在动画结束之后做，要写在这个位置
