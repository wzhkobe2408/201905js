...
// 类库、插件、UI组件、框架
// 1. 类库：提供了很多的工具方法，这里面没有业务逻辑；我们可以用这些工具方法来实现我们的业务逻辑；常用的类库：jQuery、zepto、lodash...

// 2. 插件：封装了指定功能的js代码的单元。插件都是有固定功能的，如选项卡插件，轮播图插件、日历插件；

// 3. UI组件，把固定功能的html、css、js都封装起来；需要某个功能时，直接导入就能用；常用的有antd(ant-design) / ElementUI / material-Design

// 4. 框架：有自己的设计理念和思想（数据驱动MVC、MVVM），使用框架开发需要我们按照框架思想和理念开发，然后框架会帮我们实现成应用；常用的框架React（React Native）、Vue（尤雨溪）、Backbone、Angular

// jQuery 是类库，是一个类（Function）；常用$表示jQuery；

// 1. 用jQuery获取元素对象；jq（jQuery的简称）
...
// 后代选择器：#tabBox div
// console.log($('#tabBox div')); // 获取获取id为tabBox的后代中所有的div
...
// 通过jq获取来的都是jq的实例，就能够调用jq原型上的方法。jq会把所有获取来的元素对象都放到实例中，并且有索引有length；索引对应的值都是原生对象；

// 原生对象：通过原生js提供的方法（getElementsByXXXX）获取来的元素对象；
...
// ?? 为啥要互转？因为原生对象只能原生的方法，jq对象只能调用jq原型的方法。有的时候我们需要把原生对象转成jq对象，以便使用jq原型上的方法；
