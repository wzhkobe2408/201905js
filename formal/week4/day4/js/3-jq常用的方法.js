// jq常用的方法，jq的常用方法大部分都定义在jq的原型上，也有一部分定义在了jq自身上；定义在原型上的方法只有jq的实例能调用，定义在jq自身上的方法只有jq自己可以调用；
...
// 2. width() 不传参的时候是获取元素宽度，传参是设置元素的宽度
...
// 6. outerWidth() / outerHeight() 获取元素的包含边框的宽度 相当于 offsetWidth offsetHeight
...
// 10. append() 向元素末尾追加元素
let newDiv = $('<div>这是一个新建的div</div>'); // 向$() 传一个长的像标签的字符串，会创建一个新的jq元素对象【不是原生对象】
// console.log(newDiv);
...
// 15. prev() 获取当前元素的上一个哥哥 / prevAll() 获取当前元素所有的哥哥元素
let $pp = $('.pp');
// console.log($pp.prev());
// console.log($pp.prevAll());

// 16. siblings() 获取当前元素的兄弟们
