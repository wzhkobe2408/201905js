...
$.ajax({
  ...
  success (data) { // ajax无论异步还是同步执行如果成功都会执行success；但是异步执行时，$.ajax() 后面的代码不会等着ajax结束。
    console.log(data);
    // 在这里面执行绑定数据的方法
    bindHTML(data);  // bindHTML方法结束后，$focusList重新赋值，不再是null；
...
function bindHTML(data) {
  ...
  // 为了无缝轮播：需要给wrapper最后多拼接一个第一张图片，data里面的第一条数据就是第一张图片；
...
function autoMove() {
  ...
  // stepIndex = $slideList.length - 1时，container中展示的是最后一张图片（因为最后一张图片是复制的第一张图片，用户看到的是第一张图）；
  if (stepIndex >= $slideList.length) {
    $wrapper.css('left', 0); // 此时我们把wrapper的left值设置成0，container展示的还是第一张图片；再轮播就该轮播第二张，第二张图片的索引为1，所以stepIndex的值应该设置为1；
    stepIndex = 1;
  }
...
function handleArrow() {
  // 6.1 右边的箭头的功能就是去播下一张，我们的autoMove方法专门用来播下一张的；
  $arrowRight.click(autoMove); // click方法传一个回调函数，而autoMove是函数名，代表一个函数
  $arrowLeft.click(function () {
    // 点击左侧箭头切换到上一张，stepIndex控制着播那张图，stepIndex++是播下一张，stepIndex--就是播上一张
    stepIndex--; // stepIndex减到0的时候，container里面展示的就是第一张图片了，如果再减我们需要展示真实的最后一张图片的内容（因为前面为了实现无缝轮播，我们把第一张图片复制了一份放到了wrapper的末尾，length - 1 的展示还是第一张图片）
    if (stepIndex < 0) {
...
// 7. 处理焦点的点击事件：点击第一个小点，container展示第一张图片，点击第二个小点，container展示第二张图片......，而stepIndex控制着播放图片的索引，所以我们stepIndex设置成被点击小点的索引；
...
// handleFocus(); 因为ajax是异步的，这里执行 handleFocus() 是同步执行，此时$focusList还是null，再给null绑定事件就会报错；
