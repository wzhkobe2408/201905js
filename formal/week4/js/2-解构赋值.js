...
// 1. 对数组解构
// 利用索引位置对数组进行解构取值；解构数组，等号左边要用方括号；
let ary = [1, 3, 5];
...
// 解构赋值的默认值：
let ary4 = [1, 3, 5];
let [i, j, k, l = 0] = ary4; // 对应位置上没有值或者对应位置上的值是undefined时默认值才会生效；
...
// let [m = n, n] = [undefined, 1]; 当m解构到是undefined时，需要使用n的值时，发现n尚未声明，报错（不推荐这么写）

// 对象解构赋值：
// 对象的解构赋值：利用的变量名和属性名一样，快速从对象中取值；解构对象时等号左侧要使用 花括号
...
// 函数的实参解构赋值：
function fn({ele, width = 200, height = 100, backgroundColor: bgColor}) {
  // 从形参的位置上给实参解构赋值，解构出来的都是形参
  console.log(ele, width, height, bgColor);
}
fn({
  ele: '#box',
  backgroundColor: 'red'
