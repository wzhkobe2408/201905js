...
// document.documentElement指的是整个html，document.body是整个body。他们俩的clientWidth恰好是浏览器可视窗口的宽度。

// offset
/*
* offsetWidth：clientWidth（内容宽+左右padding） + 左右边框
* offsetHeight：clientHeight(内容高 + 上下padding) + 加上下边框
...
* scrollHeight: 不溢出时就是clientHeight
* 内容溢出时：
* scrollWidth = clientWidth + scrollLeft的最大值
* scrollHeight = clientHeight + scrollTop的最大值
* */

// 图片懒加载：img标签有一个src属性，它的值存储的是图片的资源路径（url：统一资源定位符）。当浏览器检测到img标签有src属性就会根据url去加载这张图片，如果加载成功了，这个img标签就会被渲染成一张图片。延时加载（或者懒加载），不是直接给图片src属性，而是先用自定义属性的方式把图片的url存起来，等到合适的时机（一般都是这张图片即将进入浏览器可视窗口时）然后再把url取出来赋值给img的src属性即可。
// 关键点：计算图片何时出现在可视窗口；
// 图片的上外边距离body上内边缘的距离 - 页面滚动条卷去的高度 - 浏览器可视窗口的高度 <= 0 的时候图片就要出来了

// 多张图片懒加载和单张的懒加载相比：要注意不要重复加载；判断图片的src是否为空，如果不为空，就说明加载过了，不要重复加载它了。
