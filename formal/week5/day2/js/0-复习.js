...
// 原型链继承：把父类私有的和公有的都成为子类公有的。


// 2. call继承（借用构造函数继承）：
function C() {
  this.text = 'C的私有属性text';
  // call继承时this被修改为D的实例：D的实例.text = 'C的...'
}
...
function D() {
  C.call(this); // call继承：在子类的构造函数中call执行父类的构造函数，把父类构造函数中的this修改为子类的实例（D函数中的this就是当前D的实例）；
}
...
// 组合继承的缺点：会多继承一份父类私有的；第一次来自于原型链继承时，把父类私有的放到子类的原型上；第二次来自call继承时把父类私有的变成了子类私有的。此时，子类的原型上有一份父类私有的，在子类私有的也有一份父类私有的；
...
// Object.create(protoObj) 返回了一个指定了__proto__ 的新对象，这个新对象的__proto__ 指向protoObj

H.prototype = Object.create(G.prototype);
H.prototype.constructor = H; // 只要改原型对象就需要重新指定constructor
...
// 6. 冒充对象继承：在子类的构造函数中创建父类的一个实例，然后for in 循环遍历这个实例，把实例上的属性名和属性值添加到子类的实例身上；
// 冒充对象继承：把父类私有和公有的都继承为子类私有的；
...
class Human {
  constructor (name, age, sex) {
    // 在constructor里面this.xxx = xx 给实例添加私有属性
    this.name = name;
    this.age = age;
    this.sex = sex === 0 ? '男' : '女';
  }
  ...
  think () {
    console.log('我是谁，我在哪儿，我在干啥，我要去哪儿');
  }
}

class Teacher extends Human { // 1. 在声明类的时候就使用extends关键字
