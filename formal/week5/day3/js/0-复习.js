...
/*
* 1. 事件：元素（普通html元素、document）浏览器窗口（window）发生的特定的交互瞬间；通常我们监听某一个事件，以便在事件触发时可以进行我们需要的操作；
* 2. 常见的事件：
*   2.1 鼠标事件
*   onclick 点击时触发
*   ondblclick 双击时触发
...
*   window.onresize 浏览器窗口大小发生改变、或者页面被缩放时触发
*   window.onscroll 浏览器滚动条滚动的时候触发
*
*   2.4 表单事件
*   oninput input、textarea可以输入的元素的输入事件
*   onfocus 得焦事件
*   onblur 失去焦点事件
*   onchange 表单的值发生改变时触发
*   onsubmit form表单提交的时候会触发
* */

/*
* 2. 事件对象：在事件触发的时候，浏览器会执行我们的事件函数，并且传递一个实参进来，这个实参是一个对象，这个对象中包含了事件触发的信息，这个对象称为事件对象；
*   2.1 MouseEvent：
...
submit.onclick = function (e) {
  ...
  console.log(e.target.nodeName); // 获取事件源的标签名
};
