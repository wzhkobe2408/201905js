...
* 思路：放大镜放大3倍
*
* 1. 有两个大盒子，一个用来装原图的box1，另一个是用来装大图的box2；box1和box2是宽高是相等的；
* 2. box1下面有一个遮罩层mask，这个遮罩层盖住box1部分和box2露出的大图的部分是相同的，所以有一个比例关系：mask的宽高 / box1的宽高 = box2 / 大图片的尺寸；
...
* 6. 在mask在box1中移动的时候，大图片bigImg要向相反的方向移动；如果mask移动x，那么bigImg需要移动 -3x的距离；
* 7. 当鼠标移出box1的时候，mask和box2消失，所以需要监听box1的leave事件，在事件函数中把mask和box2的display设置为none
*
* */
...
// 这些都是mask移动边界值（left、top的最大值和最小值）
let minL = 0;
let minT = 0;
