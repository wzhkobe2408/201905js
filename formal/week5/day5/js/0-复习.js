// 拖拽：页面中有一个盒子，在这个盒子上按下鼠标左键不松开，移动鼠标盒子要跟随鼠标，到达目的地后松开鼠标，盒子停留在松开鼠标的位置，鼠标再移动，盒子不会再跟随；
...
function dragMove(e) {
  ...
  // 盒子的left值是 从按下鼠标到移动一段距离后 两次盒子之间的距离 + 起始的left值；
  // top 同理
...
// 2.7 ondrop 当再放置元素上松开鼠标时触发；要想用这个事件需要在dragover中阻止默认行为；
// 当向放置元素中拖入文件时，阻止浏览器打开文件，需要在ondrop中阻止默认行为；
put.ondrop = function () {};

// 发布订阅：

// 事件（广义）：所有可以量化的时刻都是一个事件；
// DOM2级事件：给每个元素每个事件都设置一个事件池，addEventListener就是订阅这个事件，把事件发生时要做的事情添加到事件池中；
// removeEventListener是取消订阅，把某个订阅了这个事件的函数从事件池中移除掉；
// DOM2事件触发时，浏览器会把事件池中的所有订阅了该事件的函数都执行；

// 发布订阅就是模拟DOM2事件的事件池思想，解决自定义事件的。准备一个数组当做事件池，并且提供订阅事件和取消订阅的方法，并且还要提供一个执行事件池中的函数的方法（DOM2事件是内置的事件，事件发生时浏览器帮你执行事件函数，但是我们自己定义的事件，我们自己去执行事件函数）；
