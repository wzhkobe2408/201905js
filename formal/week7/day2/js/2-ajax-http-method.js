...
xhr.open('GET', 'banner.json?name=mb&age=18&job=teacher&clear_cache=' + rand, true);
xhr.onreadystatechange = function () {
  if (xhr.readyState === 4 && xhr.status === 200) {
    // xhr.readyState为4表示已经彻底的结束；
  }
};
xhr.send();// send() 方法执行的时候还可以传参，这些参数是放在请求体中的
...
// => 1. GET GET请求用于从服务器获取数据，但是也可以少量向服务器传递数据；
// GET 请求的特点：

// 1.1 GET请求通过向url末尾问号传参的形式向服务器传递数据，形如?name=mb&age=18&job=teacher
// 向url末尾拼接参数（这些参数也叫查询字符串）时，如果原有的url中有?，就不需要再写问号，直接用&符号和之前的分隔；
// 这些参数以key=value的形式存在，key一般都是服务端的人定好的，人家叫咱传啥名就传啥

// 1.2 因为GET发送数据给服务端，都是把数据拼接在url末尾，但是url的总长度是有限的；一般IE是2k，Chrome是8k；总之GET请求发送的数据很小；

// 1.3 GET请求传递数据是以明文的形式传递，不安全；
// 1.4 GET请求容易产生缓存，一般情况下访问接口的数据不需要缓存；一般情况下，我们在url末尾拼接随机数或者时间戳来清除缓存；（是因为浏览器如果发现url变化了，就不会用缓存的内容，而是重新请求）；
...
/*
* 示例：
* 接口：/home/help/search 【第一个 / 表示的是www.itouzi.com】
* 功能：爱投资帮助中心根据输入关键词查询相关内容
* 请求方式：GET
* 参数：
* keyword string 用户输入关键字
* type 0 类型
* limit 10 每一页的条数
*
* 返回值：
