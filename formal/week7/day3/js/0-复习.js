...
xhr.onreadystatechange = function () {
  // 异步的状态下，xhr的readyState发生变化时会触发这个事件；
...
// 0: 表示创建ajax实例成功
// 1: 已经调用open方法，打开接口
// 2: 已经接收到响应头(如果只获取响应头中的内容，readyState为2的时候就可以)
// 3: 正在接受响应体中的内容
// 4: ajax请求彻底完成
...
// -> GET 请求的特点
// 1. 以问号传参的形式把数据传递给服务器，形如?key1=value1&key2=value2 (通过location.search 获取地址栏里面url的上的问号传参)

// 2. 因为url的长度有限，get请求能传递的数据有限；
// 3. get请求时把数据放在url末尾，以明文形式传递，不安全；
// 4. get请求容易产生缓存；（清缓存，在url末尾加一个时间戳或者随机数；原理就是浏览器检测到url变化后，不会再使用缓存，而是重新请求数据）；

// POST：通常用来向服务端传递数据（但是也可以用来获取）；
// -> POST的特点：
// 1. post 请求把数据放到请求体中传递给服务器；
...
// 2xx: 表示成功
// 200: OK 完全成功
// 204: No Content 响应成功，但是没有内容给客户端
...
// 404: Not Found 请求的资源不存在（首先确认咱们写的接口和接口文档上一致）

// 5xx: 服务端错误（遇到以5打头的错误需要服务端解决）
// 500: Internal Server Error 服务器内部错误
// 502: Bad Gateway 网关错误

// AJAX同步异步：真实项目中用异步用的更多；
// 用异步的方式请求两个接口，首先请求aside.json，等上一个结束后再请求banner.json；
