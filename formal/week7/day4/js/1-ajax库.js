...
/*$.ajax({
  url: 'banner.json',
  type: 'GET', // http-method GET POST DELETE PUT HEAD OPTIONS PATCH TRACE
 ...
 /!* error: function (err) {
  ...
  error(err) {
    // 对象的简洁语法，可以把 : function 省略；省略后error仍然是对象的一个属性，error的属性值仍然是一个函数；
    // 这个error 是在网络请求失败后执行；http-status-code 表示http请求是失败还是成功，4xx和5xx是失败；

    // 网络请求失败和数据失败是两个概念，网络请求失败根本拿不到数据；
    // 数据失败时网络请求是成功的，只不过服务器响应给我们的东西不是我们想要的；
  },
  success (data) {
    console.log(data);
  }
...
ajax({
  ...
  dataType: 'json',
  data: {
    // 做一个优化，无论get还是post在调用ajax方法时都传入键值对；在ajax内部要做处理，GET请求时把键值对变成问号传参，POST把对象转成JSON格式的字符串
    key1: 'value1',
    key2: 'value2'
  },
  error (err) {
...
function ajax({ url = '/', type = 'GET', async = true, cache = false, dataType = 'json', data = null, error, success }) {
  ...
  if (type.toUpperCase() === 'GET') {
    ...
    data = null; // 因为下面send是还会把data发送给服务器，但是get请求不需要这些东西，所以把data置为null
  } else {
