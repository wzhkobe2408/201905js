...
async function fn() {
 ...
 // console.log(x);
  ...
  // h().then((res) => console.log(res)) 有了async和await，就不用写then了；因为await可以取得promise resolve时传入的值
  ...
  // 3. await会把await下面的代码变成微任务；
}
fn();

// 真实的项目中，async和await常常结合AJAX和Promise一起使用；

// 用async和await改写先请求login.json再请求banner.json，并且在请求结束后渲染到页面中；
...
async function bindHTML() {
  ...
  // 从登录信息中取得token，把token传给getBanner; （第二个接口依赖第一个接口中的内容，用await等前面的接口响应完，就可以拿到数据，然后把你想要的数据从里面拿出来，再传给下一个请求）
