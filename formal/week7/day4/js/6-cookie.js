...
// 为了解决这个问题，http协议发明了cookie；cookie是http协议的一部分，既不属于客户端技术也不属于服务端技术；但是客户端和服务端都有操作cookie的技术；

// cookie是用来保存http状态；

// cookie是存在客户端的，但是客户端和服务端都可以操作cookie；在客户端发起请求的时候，http协议会从客户端把所有的cookie读取出来，然后带着这些cookie去请求服务器；服务器收到请求后，服务器收到请求里面是包含cookie的，服务器就可以任意操作cookie了。服务器响应的时候会让http协议把cookie再带给客户端（浏览器检测到响应中有cookie，浏览器会自动把cookie保存起来）；

// 以登录为例，第一次我们带着用户的用户名和密码去请求服务器，服务端拿到用户名和密码去数据库中匹配，如果匹配成功就成功登录，于此同时服务端会在cookie中设置一个值表示登录成功，等响应的时候这个cookie就会带到客户端，浏览器会自动把它存起来；下一次再请求的时候，http协议会自动带着cookie去请求；
...
document.cookie = 'name=bingo;path=/'; // 如果同名cookie配置属性不同不能互相覆盖；
// expires cookie的过期时间；cookie是有时效性的，如果过期，浏览器就会删除这个cookie；expires 的值是一个GMT时间；
...
// * http-only: 只能给http协议使用，前端不能获取也不能修改；（服务端设置）
