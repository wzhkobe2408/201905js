...
let server = http.createServer(function (request, response) {
  ...
  // 把index.html 响应给客户端
  // 首先我们需要把index.html 读出来，然后把读取的结果作为响应内容发送给客户端；
  ...
  // 我们在响应客户端请求的时候，现在并没有判断客户端请求的是什么；当客户端收到index.html以后会解析html，解析的时候遇到script标签，而script会再向服务器发送请求，而此时我们的服务只会响应index.html的内容，所以script收到的就是index.html的内容；
});
...
// 为什么用localhost？因为正常的请求是访问url，然后DNS负责把url解析成ip地址，然后请求ip地址；但是服务是运行在咱们的本地，本地的ip 127.0.0.1, 而127.0.0.1 就是localhost
