// 当用户请求的路径不存在的时候，返回状态码404，并且返回NOT FOUND文字；
...
let server = http.createServer((req, res) => {
  ...
  fs.stat(__dirname + pathname, (err, stats) => {
    ...
    if (err) {
      ...
      res.statusCode = 404; // res.statusCode 是一个表示HTTP status的属性，通过修改这个属性，来设置HTTP-STATUS; 404 表示请求的资源不存在；
      ...
      // 因为访问localhost:8000 时pathname是 / ，此时需要返回index.html
      if (pathname === '/') {
