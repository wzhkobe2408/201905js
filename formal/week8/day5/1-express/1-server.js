...
// express 是node的第三方依赖，需要安装然后导入使用；

// 1. 安装express
// npm install express --save 或者 yarn add express --save

// 2. 导入express
let express = require('express'); // express是第三方的依赖，不用写路径
let fs = require('fs');
...
// 4. 提供静态资源服务
app.use(express.static(__dirname)); // app.use() 是express使用中间件的方法，中间件可以理解成工具，使用这个工具就可以让咱们的服务有了某个功能；
// express.static(path) express提供静态资源服务的中间件；path是一个路径，告知express静态资源文件保存的目录；
app.use(bodyParser.json()); // 使用body-parser 解析post请求发送过来的参数，body-parser解析完会在req增加一个body属性，body属性值存储了所有的post数据（body-parser只能解析json或者form表单数据，不能解析二进制文件流）

// 5. 配置路由（写接口）

// 5.1 GET 请求
app.get('/api/getUser', function (req, res) {
  // req 和 res是express包装过的对象
  // req.query 客户端get请求的参数，是一个对象数据类型的值
  ...
  d.data = itemById;
  res.send(d); // res.send() 方法和原生end不同，send可以直接传对象
});

// 5.2 POST 请求
app.post('/api/register', function (req, res) {
  // express post请求获取客户端post过来的数据需要使用body-parser中间件
  // 使用body-parser：
  // 1. 第三方的依赖，需要安装：npm install body-parser --save
  // 2. 导入
  // 3. app.use(中间件)
  // 4. 通过req.body 获取post的数据
...
// 5.3 无论post和get 请求都会处理：
app.all('/abc/xyz', function (req, res, next) {
  // 这个回调，无论get还是post请求都会执行
