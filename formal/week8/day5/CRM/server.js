...
let app = http.createServer((req, res) => {
  ...
  if (pathname === '/' || /(\.\w+)$/.test(pathname)) {
    ...
    if (pathname === '/api/removeInfo') {
      ...
      let delIndex = con.findIndex(item => +item.id === +id); // 利用findIndex找到指定id的这一项的索引；findIndex ，返回满足条件的数组项的索引，如果没有找到符合条件返回 -1
    ...
    if (pathname === '/api/modifyInfo') {
      ...
      req.on('end', () => {
        ...
        let itemIndex = con.findIndex(item => +item.id === +data.id); // 找到你要修改的那一条数据的索引
        data.id = +data.id;
