// Node.js 是一个基于V8引擎的让js运行在服务端的运行环境（runtime）；它不是一门语言，用js开发；

// Node.js 的模块化：解决多个js互相依赖（b.js 要使用 a.js文件中的方法或者属性）；
// a.js 中要把其他模块需要的方法或者属性导出
function fn() {}
function sum() {}
// module.exports = fn; // 这样导出，直接导出的就是fn这个函数本身，如果导入a.js模块，得到的也是fn函数；

// exports.fn = fn; // 这样导出，直接导出的是一个对象，对象中有一个fn属性，它的值是fn这个函数；如果导入a.js 得到一个对象，需要 对象.fn 调用fn函数；
...
// 新建的项目（还没有安装依赖的项目），需要首先在项目中生成一个package.json文件
// 开发依赖：只在开发的时候有用，大多数情况下都是一些工具性的东西，webpack；package.json -> devDependencies
// 生产依赖：代码在运行的时候需要的依赖就是生产依赖；package.json -> dependencies
...
// node_modules 文件夹
// 我们通过npm或者yarn安装的依赖都会安装到node_modules
// 真实项目中，项目在发版上线时并不会上传node_modules，而是到了服务器上，执行npm install ，这个命令会按照package.json文件中的记录的依赖重新安装所有的依赖；
...
// 2.4 fs.writeFileSync(path, data, option) 同步写入 ; 不管是同步写入还是异步写入，都是覆盖式写入；
