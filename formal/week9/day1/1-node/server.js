...
let app = http.createServer((req, res) => {
  ...
  if (pathname === '/' || /(\.\w+)$/.test(pathname)) {
    ...
    fs.readFile(filePath, (err, data) => {
      if (err) {
        res.statusCode = 404;
        // res.end('Not Found'); // 项目中如果请求内容404，还会返回一个漂亮页面怎么做到的？当发现请求内容不存在的时候，不是直接返回 'not found'单词，而是去读取一个404的页面，然后把这个404的页面返回给客户端，客户端就会渲染成一个404页面；
    ...
    // get 请求
    // 获取所有客户信息：客户端请求接口，返回custom.json 中所有的客户信息；传了id，返回指定id的信息，不传返回全部
